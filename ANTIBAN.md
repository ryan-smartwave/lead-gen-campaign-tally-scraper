...
6. **Randomized order and timing.** Hashtags are shuffled each run, and no run starts at a predictable moment: every run begins with a randomized delay (`safety.startJitterMs`, default 0–10 minutes), so even clicking Run at 9:00 sharp every morning — or wiring up a scheduler — cannot produce a fixed daily rhythm. When a business tracks more hashtags than the per-run cap, the run rotates through the least recently scraped first, so the extra hashtags are still covered without raising the per-run volume.
